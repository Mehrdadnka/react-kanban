...
import { Task, Attachment, ActivityLog, createDefaultTask, TaskType } from '@/types/task.types';
    ...
    (set, get) => ({
      ...
      updateTask: (id, updates) => {
        set(state => ({
          tasks: state.tasks.map(task => {
            ...
            if (updates.title && updates.title !== task.title) logs.push(createLog(id, 'updated', 'title', task.title, updates.title));
            if (updates.priority && updates.priority !== task.priority) logs.push(createLog(id, 'priority_changed', 'priority', task.priority, updates.priority));
            if (updates.dueDate && updates.dueDate !== task.dueDate) logs.push(createLog(id, 'due_date_changed', 'dueDate', task.dueDate?.toISOString(), updates.dueDate?.toISOString()));
            if (updates.columnId && updates.columnId !== task.columnId) logs.push(createLog(id, 'moved', 'column', task.columnId, updates.columnId));
      ...
      deleteTask: (id) => {
        ...
        set(state => {
          ...
          if (task?.parentId) {
            tasks = tasks.map(t => t.id === task.parentId ? { ...t, subTasks: t.subTasks.filter(stId => stId !== id) } : t);
          }
          if (task?.subTasks?.length) {
            const idsToRemove = new Set(task.subTasks);
            tasks = tasks.filter(t => !idsToRemove.has(t.id));
          }
          // Remove from relatedTasks of others
          tasks = tasks.map(t => ({
            ...t,
      ...
      moveTask: (id, newColumnId, newOrder?) => {
        set(state => ({
          tasks: state.tasks.map(task => {
            ...
            return { ...task, ...updates, activityLog: [...task.activityLog, log].slice(-50) };
          }),
        }));
      },
      ...
      reorderTasks: (activeId, overId) => {
        set(state => {
          ...
          if (activeIndex !== -1 && overIndex !== -1) {
            [tasks[activeIndex].order, tasks[overIndex].order] = [tasks[overIndex].order, tasks[activeIndex].order];
            tasks[activeIndex].updatedAt = new Date();
            tasks[overIndex].updatedAt = new Date();
          }
          return { tasks };
        });
      },
      ...
      addRelatedTask: (taskId, relatedId) => {
        ...
        set(state => ({
          tasks: state.tasks.map(t => {
            if (t.id === taskId) {
              if (t.relatedTasks.includes(relatedId)) return t;
              return {
                ...t,
                relatedTasks: [...t.relatedTasks, relatedId],
                activityLog: [...t.activityLog, createLog(taskId, 'related_task_added', 'relatedTasks', '', relatedId)].slice(-50),
                updatedAt: new Date(),
              };
            }
            if (t.id === relatedId) {
              if (t.relatedTasks.includes(taskId)) return t;
              return {
                ...t,
                relatedTasks: [...t.relatedTasks, taskId],
                activityLog: [...t.activityLog, createLog(relatedId, 'related_task_added', 'relatedTasks', '', taskId)].slice(-50),
                updatedAt: new Date(),
              };
            }
            return t;
          ...
          tasks: state.tasks.map(t => {
            if (t.id === taskId) {
              return {
                ...
                activityLog: [...t.activityLog, createLog(taskId, 'related_task_removed', 'relatedTasks', relatedId, '')].slice(-50),
                updatedAt: new Date(),
              };
            }
            if (t.id === relatedId) {
              return {
                ...
                activityLog: [...t.activityLog, createLog(relatedId, 'related_task_removed', 'relatedTasks', taskId, '')].slice(-50),
                updatedAt: new Date(),
              };
            }
            return t;
      ...
      removeAttachment: (taskId, attachmentId) => {
        set(state => ({
          tasks: state.tasks.map(t => {
            if (t.id !== taskId) return t;
            return { ...t, attachments: t.attachments.filter(a => a.id !== attachmentId), updatedAt: new Date() };
          }),
        }));
      },
